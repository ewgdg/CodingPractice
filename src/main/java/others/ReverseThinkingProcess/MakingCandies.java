package others.ReverseThinkingProcess;
...
public class MakingCandies {
    ...
    public static boolean verify(long m, long w, long p, long n, long day) {
        ...
        while (true) {
            ...
            if (Math.ceil((double) (n - cur) / m / w) <= day) { // (n-cur + (m*w-1) ) / m*w , m*w-1/m*w = 0.99999 which
                                                                // will add 1 to the final result if there are remainder
                return true;
            }
            ...
            {
                // buy one m or w and then try again, bc we know if we never buy we never reach
                // goal
                if (m <= w) {
                    m++;
                } else {
            ...
            if (day <= 0) {// important , if use day<0 --> long long loop for cur -=p to decrease to 0 when
                           // p is small and cur is very large and close to n and day==0;
                return false;
            }
    ...
    // while(true){
    //// System.out.println(cur);
    // if( Math.ceil( bign.subtract( cur).divide( bigm,MathContext.DECIMAL32
    // ).divide( bigw, MathContext.DECIMAL32 ).doubleValue() )<= day ){ // (n-cur +
    // (m*w-1) ) / m*w , m*w-1/m*w = 0.99999 which will add 1 to the final result if
    // there are remainder
    // return true;
    // }
    //
    ...
    //
    // //next buy
    // long rnd = (long)Math.ceil(
    // (bigp.subtract(cur)).divide(bigm,MathContext.DECIMAL32).divide(bigw,MathContext.DECIMAL32).doubleValue()
    // ) ;
    ...
    // }
    // {
    // //buy one m or w and then try again, bc we know if we never buy we never
    // reach goal
