...
public class MedianFinderII {// <T extends Comparable<T>> if we want generic？？wrong!!!! the getmedian return
                             // l+r/2 , must be number so T entends Number & Compa
    // use treeMap
    // doesnt work with count, need to know the prefix sum of buckets
    // actually it might work with count, we just need extra position index telling
    // its position within the same elements
    ...
    MultiSet.Node<Integer> l, r; // for a better naming we can create an interface call bagIterator then node
                                 // implements it
    int size;
    ...
    public void addNumber(int num) {
        ...
        if (size == 0) {
            ...
            MultiSet.Node<Integer> node = set.add(num);
    ...
    public void remove(int num) {
        ...
        if ((size & 1) == 0) { // before remove it is even
            if (num >= r.data) {// corner case , size=2 . l = r = same data, remove same data, so it will remove
                                // l then wrong!!!!!!!!
