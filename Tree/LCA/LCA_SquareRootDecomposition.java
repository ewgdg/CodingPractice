...
//    Time Complexity for answering each query: SquareRoot of N
    ...
    //In this method we would be dividing our tree into equal-sized parts to make the process of answering queries faster



//    Now, in the preprocessing step for each node , we store a node (critical_ancestor)
//    such that it is an ancestor of the current node and it is located at the bottom-most level of the previous/upper section of the current node
...
//        Map<Integer, Set<Integer>> tree;
        ...
        traverse( 0,0,1, tree,parents,levels,critical_ancestors );


    }
    //helper to find   cri_ancestor for each node
    public void traverse(int cur,  int ancestor_head,  int prev_section ,Map<Integer,Set<Integer>> tree, Map<Integer,Integer> parents, Map<Integer,Integer> levels,Map<Integer,Integer> critical_ancestors ){
        //ancestor_head is the top ancestor of cur that belongs to the same section

//        int current_section = (int) Math.sqrt( levels.get(cur)  );

        //to find the correct ancestor node, we need to find  when first time the section is changed
        ...
        // otherwise
        //         we can set ancestor to ancestor of cur.parent. // cur and cur.parent share the same ancestor
        ...
        int cri_ancestor = parents.get(ancestor_head);
        critical_ancestors.put(cur,cri_ancestor);

        //dfs
        for(int child: tree.getOrDefault(cur, new HashSet<>())){
            if(child!=parents.get(cur)) {
                traverse(cur, ancestor_head, current_section, tree, parents, levels, critical_ancestors);
            }
        }
